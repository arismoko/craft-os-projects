...
function View:drawScreen()
    if self.activeWindow then
        ...
        term.clear()
        for i = 1, self.screenHeight - Model.statusBarHeight do  -- Adjust for status bar height
            self:drawLine(i)
        end
        self:drawStatusBar()
...
function View:drawLine(y)
    ...
    if Model.buffer[lineIndex] then
        ...
        if Model.isVisualMode and Model.visualStartY and lineIndex >= math.min(Model.visualStartY, Model.cursorY) and lineIndex <= math.max(Model.visualStartY, Model.cursorY) then
            ...
            -- Split the line into three parts: before the highlight, the highlighted part, and after the highlight
            ...
            -- Draw the line with appropriate highlighting
            term.write(beforeHighlight)
            term.setBackgroundColor(colors.gray) -- Highlight color
...
function View:drawStatusBar()
    ...
    -- Render the status message on the first line of the status bar
    if statusBarLines > 1 then
        term.setCursorPos(1, self.screenHeight - statusBarLines + 1)
    ...
    -- Render the file info and cursor position on the last line of the status bar
...
function View:updateCursor()
    local lineNumberWidth = self:getLineNumberWidth() + 1 -- +1 for the space after the line number
    local lineContent = Model.buffer[Model.cursorY] or ""

    -- Find the position of the first non-whitespace character in the line
    local firstNonWhitespacePos = lineContent:find("%S")

    if firstNonWhitespacePos then
        -- Move the cursor to the nearest non-whitespace character
        Model.cursorX = math.max(Model.cursorX, firstNonWhitespacePos)
    else
        -- If the line is empty or only whitespace, move the cursor to the first position
        Model.cursorX = 1
    end
...
function View:showAutocompleteWindow(suggestions)
    ...
    if #suggestions > 5 then
        suggestions = {table.unpack(suggestions, 1, 5)}  -- Limit suggestions to 5
    end
...
function View:getAvailableWidth()
    local lineNumberWidth = self:getLineNumberWidth()
    return self.screenWidth - lineNumberWidth - 1 -- -1 for a space between line number and text
end
